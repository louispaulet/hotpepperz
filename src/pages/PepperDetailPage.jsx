  ...
  return (
    <div className="page-sections">
      ...
      <section className="viewport-section grid gap-4">
          ...
          <article className="panel rounded-[2rem] p-6 sm:p-8">
            ...
            <div className="mt-5 grid gap-3">
              {pepper.flavorNotes.map((note) => (
                <div key={note} className="rounded-[1.4rem] border border-white/10 bg-black/16 px-4 py-3 text-sm text-[var(--color-text-soft)]">
                  {note}
                </div>
              ))}
            </div>
        ...
        <RelatedRail title="Linked reading" items={relatedItems} />
        <SourceList sources={pepper.sources} />
      </section>
